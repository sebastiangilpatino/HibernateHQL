...
public class App {
	...
	static {
		...
		sessionFactory = HibernateUtils
				.getSessionFactory(Arrays.asList(Airline.class, Airplane.class, Airport.class, Flight.class));
	}
	...
	public static void main(String[] args) {
		...
		flightsLeavingUsaCapacityGreaterThan(500);

		// b) All airlines that use A380 airplanes
		allAirlinesUsingAirplanes("A380");

		// c) Flights using 747 planes that don't belong to Star Alliance
		flightsUsingModelPlanesandNotCompanyName("747", "Star Alliance");

		// d) All flights leaving before 12pm on 08/07/2009
		LocalDateTime dateTime = LocalDateTime.of(2009, 8, 07, 12, 00);
		allFlightsLeavingBefore(dateTime);
	...
	// a) Flights leaving USA capacity > 500
	public static void flightsLeavingUsaCapacityGreaterThan(int capacity) {
		Session session = null;
		Transaction tx = null;
		...
		try {
			...
			Query query = session
					.createQuery("from Flight f where f.origin.country = 'USA' and f.airplane.capacity > :cap");
	...
	// b) All airlines that use A380 airplanes
	public static void allAirlinesUsingAirplanes(String airplaneModel) {
		Session session = null;
		Transaction tx = null;
		try {
			session = sessionFactory.openSession();
			...
			@SuppressWarnings("unchecked")
			Query query = session
					.createQuery("select distinct  a from Airline a join a.flights f where f.airplane.model = :model");
			query.setParameter("model", airplaneModel);
	...
	// c) Flights using 747 planes that don't belong to Star Alliance
	public static void flightsUsingModelPlanesandNotCompanyName(String model, String companyName) {
		Session session = null;
		Transaction tx = null;
		try {
			session = sessionFactory.openSession();
			...
			@SuppressWarnings("unchecked")
			Query query = session
					.createQuery("from Flight f where f.airplane.model = :model and f.airline.name != :companyName");
	...
	// d) All flights leaving before 12pm on 08/07/2009
	public static void allFlightsLeavingBefore(LocalDateTime dateTime) {
		Session session = null;
		Transaction tx = null;
		try {
			session = sessionFactory.openSession();
			...
			Query query = session.createQuery(
					"from Flight f where f.departureDate = :departureDate and f.departureTime < :departureTime");

			Date queryDate = convertToDateViaSqlTimestamp(dateTime);
			query.setParameter("departureDate", queryDate);
			query.setParameter("departureTime", queryDate);
